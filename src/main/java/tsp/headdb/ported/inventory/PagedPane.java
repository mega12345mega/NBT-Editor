...
public class PagedPane extends ClientHandledScreen {
	...
	public void selectPage(int index) {
		if (index < 0 || index >= getPageAmount()) {
			throw new IllegalArgumentException("Index out of bounds s: " + index + " [" + 0 + ", " + getPageAmount() + ")");
		}
	...
	protected void onMouseClick(Slot slot, int slotId, int button, SlotActionType actionType) {
		...
		if (event.getSlotId() == getInventory().size() - 8) { // back item
			if (controlBack != null) {
				controlBack.onClick(event);
			}
			return;
		} else if (event.getSlotId() == getInventory().size() - 2) { // next item
			if (controlNext != null) {
	...
	protected void createControls(Inventory inventory) {
		// create separator
		fillRow(inventory.size() / 9 - 2, new ItemStack(Items.BLACK_STAINED_GLASS_PANE), inventory);
		
		if (getCurrentPage() > 1) {
			String name = String.format(Locale.ROOT, "&3&lPage &a&l%d &7/ &c&l%d", getCurrentPage() - 1, getPageAmount());
			String lore = String.format(Locale.ROOT, "&7Previous: &c%d", getCurrentPage() - 1);
		...
		if (getCurrentPage() < getPageAmount()) {
			String name = String.format(Locale.ROOT, "&3&lPage &a&l%d &7/ &c&l%d", getCurrentPage() + 1, getPageAmount());
			String lore = String.format(Locale.ROOT, "&7Next: &c%d", getCurrentPage() + 1);
		...
		{
			String name = String.format(Locale.ROOT, "&3&lPage &a&l%d &7/ &c&l%d", getCurrentPage(), getPageAmount());
			ItemStack itemStack = setMeta(HeadAPI.getHeadByValue("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvY2Q5MWY1MTI2NmVkZGM2MjA3ZjEyYWU4ZDdhNDljNWRiMDQxNWFkYTA0ZGFiOTJiYjc2ODZhZmRiMTdmNGQ0ZSJ9fX0=").getItemStack(),
					name, "&7Left-Click to go to the &cMain Menu", "&7Right-Click to go to a &6Specific Page");
