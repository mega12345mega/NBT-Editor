...
public interface ComponentHolderMixin extends MVComponentsAccess {
	...
	public default <T> T nbte$get(ComponentType<? extends T> type) {
		if (Impl.MOVED)
			...
			return Impl.ComponentHolder_get.get().invoke(this, type);
	}
	...
	public default <T> T nbte$getOrDefault(ComponentType<? extends T> type, T fallback) {
		if (Impl.MOVED)
			...
			return Impl.ComponentHolder_getOrDefault.get().invoke(this, type, fallback);
	}
