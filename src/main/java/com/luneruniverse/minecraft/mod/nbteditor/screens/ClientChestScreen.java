...
public class ClientChestScreen extends ClientContainerScreen {
	...
	protected void init() {
		...
		this.addDrawableChild(lockSlots = new ButtonWidget(this.x - 87, this.y + 48, 83, 20, ConfigScreen.shouldLockSlots() ? new TranslatableText("nbteditor.unlock_slots") : new TranslatableText("nbteditor.lock_slots"), btn -> {
			navigationClicked = true;
			ConfigScreen.setLockSlots(!ConfigScreen.shouldLockSlots());
			btn.setMessage(ConfigScreen.shouldLockSlots() ? new TranslatableText("nbteditor.unlock_slots") : new TranslatableText("nbteditor.lock_slots"));
		}));
		lockSlots.active = !ConfigScreen.shouldDisableLockSlotsButton();
		
		this.addDrawableChild(new ButtonWidget(this.x - 87, this.y + 72, 83, 20, new TranslatableText("nbteditor.reload_page"), btn -> {
			navigationClicked = true;
			try {
		...
		this.addDrawableChild(new ButtonWidget(this.x - 87, this.y + 96, 83, 20, new TranslatableText("nbteditor.clear_page"), btn -> {
			navigationClicked = true;
