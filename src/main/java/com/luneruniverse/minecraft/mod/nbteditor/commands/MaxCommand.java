...
public class MaxCommand implements RegisterableCommand {
	
	@Override
	public LiteralCommandNode<FabricClientCommandSource> register(boolean dedicated) {
		return ClientCommandManager.DISPATCHER.register(literal("max")
				.then(literal("cursed")
						.then(literal("all")
	...
	private int max(CommandContext<FabricClientCommandSource> context, int enchantLevel, boolean allEnchants, boolean cursed) throws CommandSyntaxException {
		...
		context.getSource().sendFeedback(new TranslatableText("nbteditor.maxed"));
