...
public class SignatureCommand implements RegisterableCommand {
	...
	static {
		if (!SIGNATURE_FILE.exists())
			signature = new TranslatableText("nbteditor.signature_default");
		else {
			try {
				signature = Text.Serializer.fromJson(new String(Files.readAllBytes(SIGNATURE_FILE.toPath())));
			} catch (IOException e) {
				e.printStackTrace();
				signature = new TranslatableText("nbteditor.signature_load_error");
			}
		}
	}
	...
	public LiteralCommandNode<FabricClientCommandSource> register(boolean dedicated) {
		Command<FabricClientCommandSource> addSignature = context -> {
			...
			if (lore.size() == 0 || !lore.getLine(-1).equals(signature))
				...
				context.getSource().sendFeedback(new TranslatableText("nbteditor.signature_already_added"));
				return Command.SINGLE_SUCCESS;
			}
			
			MainUtil.saveItem(hand, item);
			context.getSource().sendFeedback(new TranslatableText("nbteditor.signature_added"));
			
			return Command.SINGLE_SUCCESS;
		};
		
		return ClientCommandManager.DISPATCHER.register(
				literal("signature").executes(addSignature)
					...
					.then(literal("remove").executes(context -> {
						...
						Lore lore = new Lore(item);
						if (lore.size() == 0 || !lore.getLine(-1).equals(signature)) {
							context.getSource().sendFeedback(new TranslatableText("nbteditor.signature_not_found"));
							return Command.SINGLE_SUCCESS;
						}
						
						lore.removeLine(-1);
						MainUtil.saveItem(hand, item);
						context.getSource().sendFeedback(new TranslatableText("nbteditor.signature_removed"));
					...
					.then(literal("edit").then(argument("signature", TextArgumentType.text()).executes(context -> {
						...
						try {
							...
							throw new SimpleCommandExceptionType(new TranslatableText("nbteditor.signature_arg_missing")).create();
						}
						try {
							Files.write(SIGNATURE_FILE.toPath(), Text.Serializer.toJson(signature).getBytes());
						} catch (IOException e) {
							e.printStackTrace();
							throw new SimpleCommandExceptionType(new TranslatableText("nbteditor.signature_save_error")).create();
						}
						...
						context.getSource().sendFeedback(new TranslatableText("nbteditor.signature_edited"));
