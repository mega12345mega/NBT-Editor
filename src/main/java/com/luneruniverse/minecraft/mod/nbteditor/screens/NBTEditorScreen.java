...
public class NBTEditorScreen extends Screen {
	...
	protected void init() {
		if (realPath.isEmpty() && ((NbtCompound) this.nbt).contains("")) {
			...
			}, new TranslatableText("nbteditor.emptykey.title"), new TranslatableText("nbteditor.emptykey.message"),
					new TranslatableText("nbteditor.emptykey.yes"), new TranslatableText("nbteditor.emptykey.no")));
		...
		this.addDrawableChild(saveBtn = new ButtonWidget(16 + (32 + 8) * 2 + 100 + 8, 16 + 6, 100, 20, new TranslatableText("nbteditor.save"), btn -> {
			save();
		}));
		this.addDrawableChild(new ButtonWidget(16, height - 16 * 2, 20, 20, new TranslatableText("nbteditor.add"), btn -> {
			add();
		}));
		this.addDrawableChild(new ButtonWidget(16 + 16 + 8, height - 16 * 2, 20, 20, new TranslatableText("nbteditor.remove"), btn -> {
			remove();
		}));
		this.addDrawableChild(new ButtonWidget(16 + (16 + 8) * 2, height - 16 * 2, 48, 20, new TranslatableText("nbteditor.copy"), btn -> {
			copy();
		}));
		this.addDrawableChild(new ButtonWidget(16 + (16 + 8) * 2 + (48 + 4), height - 16 * 2, 48, 20, new TranslatableText("nbteditor.cut"), btn -> {
			cut();
		}));
		this.addDrawableChild(new ButtonWidget(16 + (16 + 8) * 2 + (48 + 4) * 2, height - 16 * 2, 48, 20, new TranslatableText("nbteditor.paste"), btn -> {
			paste();
		}));
		this.addDrawableChild(new ButtonWidget(16 + (16 + 8) * 2 + (48 + 4) * 3, height - 16 * 2, 48, 20, new TranslatableText("nbteditor.rename"), btn -> {
			rename();
		}));
		...
		path = new NamedTextFieldWidget(textRenderer, 16, 16 + 8 + 32 + 16 + 8, 288, 16, new TranslatableText("nbteditor.path")).name(new TranslatableText("nbteditor.path"));
		...
		value = new NamedTextFieldWidget(textRenderer, 16, 16 + 8 + 32 + (16 + 8) * 2, 288, 16, new TranslatableText("nbteditor.value")).name(new TranslatableText("nbteditor.value"));
		...
		gen.addElement(this, this.nbt, force -> {
			if (force == null)
				...
				client.setScreen(new ConfirmScreen(value -> {
					...
					client.setScreen(NBTEditorScreen.this);
				}, new TranslatableText("nbteditor.overwrite.title"), new TranslatableText("nbteditor.overwrite.message"),
						new TranslatableText("nbteditor.overwrite.yes"), new TranslatableText("nbteditor.overwrite.no")));
			}
		}, null);
	}
	private void remove() {
