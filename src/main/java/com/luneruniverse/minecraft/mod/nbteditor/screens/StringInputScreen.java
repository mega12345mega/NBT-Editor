...
public class StringInputScreen extends Screen {
	...
	protected void init() {
		...
		this.addDrawableChild(new ButtonWidget(width / 2 - 104, height / 2 + 4, 100, 20, new TranslatableText("nbteditor.ok"), btn -> {
			if (valueValidator.test(value.getText())) {
				client.setScreen(screen);
				valueConsumer.accept(value.getText());
			}
		}));
		this.addDrawableChild(new ButtonWidget(width / 2 + 4, height / 2 + 4, 100, 20, new TranslatableText("nbteditor.cancel"), btn -> {
			client.setScreen(screen);
		}));
	}
