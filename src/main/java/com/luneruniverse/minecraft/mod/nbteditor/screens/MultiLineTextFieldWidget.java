	...
	private void generateLines() {
		...
		if (formatter != null) {
			...
			formatter.apply(this.text).visit(new StyledVisitor<Boolean>() {
				private int i = 0;
				private LiteralText line = new LiteralText("");
				...
				public Optional<Boolean> accept(Style style, String str) {
					...
					if (currentLength + str.length() >= targetLength) {
						line.append(new LiteralText(str.substring(0, targetLength - currentLength)).fillStyle(style));
						formattedLines.add(line);
						line = new LiteralText("");
						i++;
						if (currentLength + str.length() > targetLength)
							accept(style, str.substring(targetLength - currentLength));
					} else
						line.append(new LiteralText(str).fillStyle(style));
